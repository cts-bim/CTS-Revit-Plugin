...
namespace CTSRevitPlugin.Commands.Modeling
{
    ...
    [ToolDocumentation(
        description: "Align selected elements using their Y-axis position.",
        usage: "Select two or more elements and run the command.",
        author: "Bruno Silva",
        version: "1.0",
        notes: "CTS standard command documentation."
    )]
    public class AlignByYCommand : IExternalCommand
    {
        ...
        public static Result Run(UIApplication uiapp)
        {
            ...
            double delta = p2.Y - p1.Y;

            if (Math.Abs(delta) < 1e-9)
                return Result.Succeeded;

            using (var t = new Transaction(doc, "Align by Y"))
            {
                t.Start();
