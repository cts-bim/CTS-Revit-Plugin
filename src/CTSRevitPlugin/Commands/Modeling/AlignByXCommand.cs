...
namespace CTSRevitPlugin.Commands.Modeling
{
    ...
    [ToolDocumentation(
        description: "Align selected elements using their X-axis position.",
        usage: "Select two or more elements and run the command.",
        author: "Bruno Silva",
        version: "1.0",
        notes: "CTS standard command documentation."
    )]
    public class AlignByXCommand : IExternalCommand
    {
        ...
        public static Result Run(UIApplication uiapp)
        {
            ...
            using (var t = new Transaction(doc, "Align by X"))
            {
                t.Start();
